    ...
    if not imag:
        ...
        data ={
                ...
                "data": {
                    ...
                    "content": [
                        {"type": "text", "data": {"text": msg}},
                        {"type": "image", "data": {"file": image(imag)}}
                        ]
                        }
            }
        chain.append(data)
...
'''
async def get_comic_from_id(bookid:str):
    error_recall = []
    ...
    try:
        ...
        await download_thumb(thumb_url, cm_name)
    except:
...
async def get_random_favorite(res:dict):
    ...
    for key,value in favor.items():
        detail = f"{value[0]}：\n漫画ID:{key}\n作者:{value[1]}"
        thumb_url = value[2]
        await download_thumb(thumb_url, value[0])
...
'''
async def get_search_bookid(result:dict):
    error_recall = []
    comic_info = []
    if result["data"]["comics"]["total"] == 0:
        ...
        try:
            ...
            chineseTeam = "Unknown"
        comic_info = f"{title}：\n漫画ID:{bookid}\n作者:{author}\n汉化:{chineseTeam}"
        await download_thumb(thumb_url, title)
        return error_recall, comic_info
...
#>>>根据关键字搜本<<<
@sv.handle()
async def search_pica(bot: Bot, ev: Event):
    chain = []
    input1 = ev.message.extract_plain_text()
    ...
    if not input1:
        ...
        #转换为繁体
        ex_input = OpenCC('s2t').convert(input1.strip())
        ...
        if os.path.exists(zippath):
            ...
            if isinstance(ev, PrivateMessageEvent):
                if str(uid) in fd_list:
                    await bot.send_private_msg(user_id=int(uid),message=sub)
            ...
            try:
                search_post = await p.search(ex_input, sort=Order_Default)
            except:
                await bot.send(ev, f"请求失败")
            
            search_post = await search_post.text()
            ...
            if not search_result["code"] == 200:
                ...
                error, comic_info = await get_search_bookid(search_result)
                #合并转发消息节点
...
svv = on_command("分区搜", block=True, priority=5)
#>>>指定分区的关键字搜本<<<
@svv.handle()
async def search_pica_cate(bot: Bot, ev: Event):
    chain = []
    uid = ev.user_id
...
#>>>使用漫画ID查看本子<<<
@svid.handle()
async def get_pica(bot: Bot, ev: Event):
    ...
    if not input3:
        ...
        if os.path.exists(zippath):
            ...
            await get_comic_from_id(input3)
...
async def get_pica_random(bot: Bot, ev: Event):
    ...
    try:
        ...
        chineseTeam = "Unknown"
    comic_info = f"{sub}：\n漫画ID:{bookid}\n作者:{author}\n汉化:{chineseTeam}"
    await download_thumb(thumb_url, sub)
    ...
    if isinstance(ev, PrivateMessageEvent):
        if str(uid) in fd_list:
            await bot.send_private_msg(user_id=int(uid),message=thb_path)
            await bot.send_private_msg(user_id=int(uid),message=comic_info)
            try:
...
#>>>指定分区的随机本子<<<
@sjz.handle()
async def get_pica_cate_random(bot: Bot, ev: Event):
    ...
    try:
        res = await p.comics(block=ex_input5, order=order)
    except:
        await bot.send(ev, f"请求失败")
    out = res["data"]["comics"]["docs"][0]
    bookid = out["_id"]
...
stj = on_command("本子推荐", block=True, priority=5)
@stj.handle()
async def get_pica_cate_random(bot: Bot, ev: Event):
    my_list = []
    uid = ev.user_id
    ...
    for loop in range(1, page_num):
        comment = await p.my_favourite(page=loop,order=Order_Default)
        comment = await comment.text()
        com_js = json.loads(comment)
        favors = await get_random_favorite(com_js)
        #遍历我的收藏,并将相关信息放入查询列表
...
async def check_pica(bot: Bot, ev: Event):
    ...
    try:
        ...
        pic2_ck = ts["code"]
        async with aiohttp.ClientSession() as session:
            rs = await session.get(url="https://storage1.picacomic.com/static/653cc9e0-1548-4cc3-bba0-d6e2e1bd9c18.jpg", proxy=proxy)
